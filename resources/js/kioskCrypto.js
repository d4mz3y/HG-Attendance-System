const ROTATE_RIGHT = (value, amount) => (value >>> amount) | (value << (32 - amount));
...
function sha256Bytes(input) {
    ...
    for (let offset = 0; offset < paddedLength; offset += 64) {
        ...
        for (let index = 16; index < 64; index += 1) {
            ...
            const sigma0 = ROTATE_RIGHT(w15, 7) ^ ROTATE_RIGHT(w15, 18) ^ (w15 >>> 3);
            const sigma1 = ROTATE_RIGHT(w2, 17) ^ ROTATE_RIGHT(w2, 19) ^ (w2 >>> 10);
            words[index] = (words[index - 16] + sigma0 + words[index - 7] + sigma1) >>> 0;
        }
        let [a, b, c, d, e, f, g, h] = hash;
        for (let index = 0; index < 64; index += 1) {
            const sum1 = ROTATE_RIGHT(e, 6) ^ ROTATE_RIGHT(e, 11) ^ ROTATE_RIGHT(e, 25);
            const choice = (e & f) ^ (~e & g);
            const temp1 = (h + sum1 + choice + constants[index] + words[index]) >>> 0;
            const sum0 = ROTATE_RIGHT(a, 2) ^ ROTATE_RIGHT(a, 13) ^ ROTATE_RIGHT(a, 22);
...
export async function hmacSha256(secret, message) {
    ...
    if (globalThis.crypto?.subtle) {
        ...
        return toHex(new Uint8Array(await globalThis.crypto.subtle.sign('HMAC', key, encoder.encode(message))));
    }
    ...
    inner.set(innerPad); inner.set(messageBytes, innerPad.length);
    const innerHash = sha256Bytes(inner);
    const outer = new Uint8Array(outerPad.length + innerHash.length);
    outer.set(outerPad); outer.set(innerHash, outerPad.length);
